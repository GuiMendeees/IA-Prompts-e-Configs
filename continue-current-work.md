O objetivo principal agpra é **atualizar** o estado atual no `current-work.md` com as informações relevantes aprendidas durate a execução que possam ser úteis para futuras subtarefas ou para o contexto geral do projeto.

> Caso existam observações adicionais, você deve rever o planejamento do `current-work.md` e criar subtarefas adicionais caso necessário.
Após isso, você deve **continuar a execução das subtarefas**, uma de cada vez, até finalizar a tarefa por completo, seguindo meticulosamente as diretrizes do **fluxo de trabalhor RIPER para cada subtarefa**
